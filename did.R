...
citations_africa <- if (file.exists("citations_progress_africa.rds")) {
  readRDS("citations_progress_africa.rds")
} else {
  tibble(
    inst_id     = character(),
    year        = integer(),
...
to_fetch <- df_africa %>%
  distinct(inst_id, year) %>%
  filter(!is.na(inst_id)) %>%
  anti_join(citations_africa, by = c("inst_id", "year"))
...
for (i in seq_len(nrow(to_fetch))) {
  ...
  citations_africa <- bind_rows(citations_africa, result)
  saveRDS(citations_africa, "citations_progress_africa.rds")
}

write_csv(citations_africa, 'citations_africa.csv')
